...
import { ChallengeRecapPayload, NAV_STATE_KEYS, readNavState, writeNavState } from '../utils/navigationState';
...
const ChallengeRecapScreen: React.FC<Props> = ({ onNavigate }) => {
  ...
  const challengeTitle = recap?.challengeTitle || 'Challenge Recap';
  const progressText = recap?.challengeProgress || '—';
  const totalMinutes = recap?.totalMinutes || '—';
  const topContributor = recap?.topContributor || '—';
  const fundraisingAmount = recap?.fundraisingAmount || '—';
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display flex flex-col antialiased">
      ...
      <main className="flex-1 overflow-y-auto pb-48">
        <div className="px-8 text-center pt-8">
          <h1 className="text-[42px] font-black tracking-tighter leading-[0.9] mb-4">{challengeTitle}</h1>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-10">
          <div className="relative flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-5xl font-black text-primary leading-none">{progressText}</span>
        ...
        <section className="px-6 space-y-4 pt-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {(recap?.highlights?.length
              ? recap.highlights
              : [
                  { label: 'Total Time', val: totalMinutes, icon: 'timer' },
                  { label: 'Top Contributor', val: topContributor, icon: 'stars' }
                ]
            ).map((stat, i) => (
              <div key={i} className="bg-white dark:bg-slate-800/50 p-6 rounded-[32px] border border-slate-50 dark:border-zinc-800 shadow-sm space-y-4">
                <div className="size-10 bg-primary/10 rounded-2xl flex items-center justify-center text-primary shadow-inner">
                  <span className="material-icons-round text-xl">{stat.icon || 'stars'}</span>
                </div>
                <div>
        ...
        <section className="px-6 mt-12 space-y-6">
          ...
          <div className="space-y-3">
            ...
            {(recap?.topContributors || []).map((p, i) => (
              <div key={p.name} className={`flex items-center gap-5 p-5 rounded-[32px] border border-slate-50 dark:border-zinc-800 shadow-sm ${i === 0 ? 'bg-primary/5 border-primary/20' : 'bg-white dark:bg-slate-900/50'}`}>
                <div className={`size-10 rounded-2xl flex items-center justify-center text-white font-black text-sm shadow-md ${p.color || 'bg-primary'}`}>{p.rank || i + 1}</div>
                <img className="size-14 rounded-[24px] object-cover ring-4 ring-primary/5" src={p.avatar || '/icons/icon-192.svg'} alt={p.name} />
          ...
          <div className="bg-gradient-to-br from-primary to-orange-600 p-8 rounded-[48px] text-white shadow-2xl shadow-primary/30 relative overflow-hidden">
            <div className="relative z-10 space-y-6">
              ...
              <p className="text-sm font-bold leading-relaxed opacity-90">
                {recap?.impactLabel || 'Community impact summary unavailable.'}
              </p>
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase tracking-widest opacity-80">Fundraising</span>
                  <span className="text-xl font-black">{fundraisingAmount} Raised</span>
                </div>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md border-t border-slate-100 dark:border-slate-800 z-50 flex flex-col gap-4">
        <button 
          onClick={() => {
            if (recap) {
              writeNavState(NAV_STATE_KEYS.challengeRecap, recap);
            }
            onNavigate(AppView.CHALLENGE_RECAP_STORY);
          }}
