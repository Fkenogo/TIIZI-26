...
const LevelUpModalScreen: React.FC<Props> = ({ onNavigate }) => {
  ...
  const level = Number(params.get('level') || 5);
  const xp = Number(params.get('xp') || 1000);
  const targetXp = Number(params.get('targetXp') || 1000);
  ...
  const { items: privileges } = useFirestoreCollection<{ id: string; title?: string; sub?: string; icon?: string }>(
    ['levelUpPrivileges']
  );
      ...
      <main className="flex-1 w-full max-w-sm px-6 flex flex-col items-center">
        ...
        <div className="relative mt-8 mb-12">
          ...
          <div className="size-56 rounded-full bg-gradient-to-br from-primary via-orange-500 to-amber-500 flex flex-col items-center justify-center border-[10px] border-white/20 shadow-[0_32px_80px_-16px_rgba(211,109,33,0.4)] relative ring-8 ring-primary/10">
             ...
             <h3 className="text-8xl font-black leading-none text-white tracking-tighter italic">{level}</h3>
          </div>
        </div>
        ...
        <div className="w-full space-y-4 mb-12">
           <div className="flex justify-between items-baseline px-2">
              <p className="text-[10px] font-black text-primary uppercase tracking-widest">Level {level} Reached</p>
              <p className="text-sm font-black text-slate-900 dark:text-white">{xp.toLocaleString()} / {targetXp.toLocaleString()} XP</p>
           </div>
        ...
        <section className="w-full space-y-6">
           ...
           <div className="space-y-3">
             {privileges.length === 0 && (
               <div className="text-sm text-slate-400 px-2">No new privileges yet.</div>
             )}
             {privileges.map((p) => (
               <div key={p.id} className="flex items-center justify-between p-6 bg-white dark:bg-white/5 rounded-[32px] border border-slate-50 dark:border-white/5 shadow-sm">
                 <div className="flex items-center gap-5">
                   <div className="size-11 rounded-2xl bg-primary/10 flex items-center justify-center text-primary shadow-inner">
                     <span className="material-icons-round">{p.icon || 'star'}</span>
                   </div>
                   <div>
