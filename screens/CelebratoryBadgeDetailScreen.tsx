...
const CelebratoryBadgeDetailScreen: React.FC<Props> = ({ onNavigate }) => {
  ...
  const badgeTitle = badge?.name || 'Badge';
  const badgeDescription = badge?.description || 'Badge details will appear once loaded from Firestore.';
  const badgeIcon = badge?.icon || 'handshake';
  const badgeRarity = badge?.rarity || 'Achievement';
  const milestoneConstraints = useMemo(() => [orderBy('date', 'asc')], []);
  const { items: milestones } = useFirestoreCollection<{
    ...
    icon?: string;
    color?: string;
  }>(state.user.authUid ? ['users', state.user.authUid, 'milestones'] : [], milestoneConstraints);
  const backView = from === 'achievements_hub' ? AppView.ACHIEVEMENTS_HUB : AppView.MODERN_ACHIEVEMENTS_HUB;
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-[#1b140d] dark:text-white transition-colors duration-300 font-display flex flex-col antialiased relative">
      ...
      <main className="flex-1 overflow-y-auto pb-48">
        ...
        <div className="relative px-6 pt-16 pb-12 flex flex-col items-center">
          <div className="relative">
            ...
            <div className="relative w-52 h-52 bg-white dark:bg-white/5 rounded-full flex items-center justify-center shadow-2xl border-4 border-primary ring-8 ring-primary/5 transition-transform hover:scale-105 duration-700">
              <span className="material-icons-round text-[110px] text-primary font-variation-fill" style={{ fontVariationSettings: "'FILL' 1" }}>{badgeIcon}</span>
            </div>
          ...
          <div className="text-center mt-12 space-y-4">
            <h1 className="text-4xl font-black tracking-tighter leading-none italic uppercase">{badgeTitle}</h1>
        ...
        <div className="px-6 py-4">
          <div className="bg-white dark:bg-white/5 p-10 rounded-[48px] shadow-xl shadow-primary/5 border-2 border-slate-50 dark:border-white/5 text-center relative overflow-hidden group">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-base leading-relaxed font-medium">{badgeDescription}</p>
          </div>
        </div>
        ...
        <div className="px-8 py-10">
          ...
          <div className="space-y-0 relative before:absolute before:left-[21px] before:top-2 before:h-[calc(100%-24px)] before:w-1 before:bg-slate-100 dark:before:bg-white/5">
            ...
            {milestones.length === 0 ? (
              ...
              milestones.map((step) => (
                <div key={step.id} className="relative pl-16 pb-12 last:pb-0">
                  <div className={`absolute left-0 top-1 size-11 rounded-2xl flex items-center justify-center text-white z-10 shadow-lg ${step.color || 'bg-primary'}`}>
                    <span className="material-icons-round text-lg">{step.icon || 'check'}</span>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{step.date || '—'}</p>
                    <h4 className="font-black text-base tracking-tight">{step.title || 'Milestone'}</h4>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-background-light via-background-light/95 to-transparent dark:from-background-dark dark:via-background-dark/95 z-50">
        <div className="max-w-md mx-auto space-y-6">
          <button 
            onClick={() => badgeId && onNavigate((`${AppView.SHARE_PROGRESS_CARD}?mode=badge&badgeId=${encodeURIComponent(badgeId)}&source=celebratory_badge_detail`) as AppView)}
