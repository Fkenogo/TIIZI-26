...
import { ChallengeRecapPayload, NAV_STATE_KEYS, writeNavState } from '../utils/navigationState';
...
const GroupHistoryScreen: React.FC<Props> = ({ onNavigate }) => {
  ...
  const { items: pastChallenges } = useFirestoreCollection<ChallengeRecapPayload>(
    state.activeGroupId ? ['groups', state.activeGroupId, 'challengeHistory'] : []
  );
  const { items: milestones } = useFirestoreCollection<{ id: string; label?: string; date?: string; icon?: string }>(
    state.activeGroupId ? ['groups', state.activeGroupId, 'milestones'] : []
  );
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-[#1b140d] dark:text-white font-display flex flex-col antialiased">
      <header className="flex items-center bg-background-light dark:bg-background-dark p-4 pt-12 pb-2 justify-between sticky top-0 z-10 border-b border-gray-100 dark:border-zinc-800">
        ...
        <div className="flex flex-col items-center flex-1">
          ...
          <p className="text-[10px] text-primary font-black uppercase tracking-widest">{activeGroup?.name || ''}</p>
        </div>
      ...
      <main className="flex-1 overflow-y-auto pb-10">
        ...
        <section className="px-5 space-y-6">
          ...
          {pastChallenges.length === 0 && (
            <div className="text-sm text-zinc-500">No completed challenges yet.</div>
          )}
          {pastChallenges.map((challenge, i) => (
            <div 
              key={i} 
              onClick={() => {
                writeNavState(NAV_STATE_KEYS.challengeRecap, challenge);
                onNavigate(AppView.CHALLENGE_RECAP);
              }}
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="text-lg font-black tracking-tight leading-tight">{challenge.challengeTitle}</h4>
                  <p className="text-[10px] font-bold text-zinc-500 mt-1 uppercase tracking-widest">{challenge.challengeDate}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <span className="text-2xl font-black text-primary leading-none">{challenge.challengeProgress} <span className="text-sm">Goal Met</span></span>
                </div>
                <div className="text-xs text-slate-400">{challenge.activeMembers} active</div>
              </div>
          ...
          <div className="space-y-3">
            ...
            {milestones.map((m) => (
              <div key={m.id} className="flex items-center gap-5 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm">
                <div className="size-14 rounded-2xl bg-primary/10 flex items-center justify-center shrink-0 shadow-inner">
                  <span className="material-icons-round text-primary text-2xl">{m.icon || 'emoji_events'}</span>
                </div>
                <div>
                  <p className="font-black text-sm leading-tight mb-1">{m.label || 'Milestone'}</p>
