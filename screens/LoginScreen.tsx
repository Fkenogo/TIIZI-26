
import React, { useState } from 'react';
import { AppView } from '../types';

interface Props {
  onNavigate: (view: AppView) => void;
}

const LoginScreen: React.FC<Props> = ({ onNavigate }) => {
  const [authMethod, setAuthMethod] = useState<'Phone' | 'Email'>('Phone');

  return (
    <div className="relative flex min-h-screen w-full flex-col bg-background-light dark:bg-background-dark font-display overflow-x-hidden">
      <header className="flex items-center p-4 pt-12 pb-2 justify-end">
        <button 
          onClick={() => onNavigate(AppView.WELCOME)}
          className="text-slate-400 hover:text-slate-600 p-2"
        >
          <span className="material-icons-round">close</span>
        </button>
      </header>

      <div className="flex flex-col items-center justify-center pt-4 pb-10">
        <div className="w-20 h-20 bg-primary rounded-3xl flex items-center justify-center mb-6 shadow-xl shadow-primary/20 scale-110">
          <span className="material-icons-round text-white text-5xl">fitness_center</span>
        </div>
        <h1 className="text-[32px] font-black text-slate-900 dark:text-white tracking-tight">Tiizi</h1>
        <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px] mt-2">Join your fitness community</p>
      </div>

      <div className="flex flex-col px-6 flex-1 max-w-sm mx-auto w-full">
        {/* Segmented Control */}
        <div className="flex h-12 bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl mb-8">
          {(['Phone', 'Email'] as const).map((method) => (
            <button
              key={method}
              onClick={() => setAuthMethod(method)}
              className={`flex-1 flex items-center justify-center rounded-xl text-xs font-black transition-all ${
                authMethod === method 
                  ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' 
                  : 'text-slate-400'
              }`}
            >
              {method}
            </button>
          ))}
        </div>

        <div className="space-y-6">
          <label className="flex flex-col w-full">
            <span className="text-sm font-black uppercase tracking-widest text-slate-400 mb-2 px-1">
              {authMethod} {authMethod === 'Phone' ? 'Number' : 'Address'}
            </span>
            <input 
              className="w-full h-16 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-800 rounded-2xl px-5 text-base font-bold focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all dark:text-white placeholder:text-slate-300" 
              placeholder={authMethod === 'Phone' ? '+1 (555) 000-0000' : 'name@example.com'}
              type={authMethod === 'Phone' ? 'tel' : 'email'}
            />
          </label>

          <button 
            onClick={() => onNavigate(AppView.VERIFY)}
            className="w-full bg-primary hover:bg-orange-600 text-white text-lg font-black h-16 rounded-2xl shadow-xl shadow-primary/20 transition-all active:scale-95"
          >
            Continue
          </button>
        </div>

        <div className="flex items-center my-10">
          <div className="flex-1 h-px bg-slate-100 dark:bg-slate-800"></div>
          <span className="px-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-300 dark:text-slate-600">Or continue with</span>
          <div className="flex-1 h-px bg-slate-100 dark:bg-slate-800"></div>
        </div>

        <div className="grid grid-cols-1 gap-4">
          <button className="flex h-16 w-full items-center justify-center gap-4 rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800/50 text-slate-700 dark:text-white text-base font-bold hover:bg-slate-50 transition-all active:scale-[0.98]">
            <svg className="w-6 h-6" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"></path>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"></path>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"></path>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"></path>
            </svg>
            Google
          </button>
          <button className="flex h-16 w-full items-center justify-center gap-4 rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800/50 text-slate-700 dark:text-white text-base font-bold hover:bg-slate-50 transition-all active:scale-[0.98]">
            <span className="material-icons-round text-2xl">apple</span>
            Apple
          </button>
        </div>
      </div>

      <div className="mt-auto pb-12 flex justify-center pt-8">
        <p className="text-slate-400 text-sm font-bold">
          Don't have an account? 
          <button 
            onClick={() => onNavigate(AppView.SIGNUP)}
            className="text-primary font-black ml-2 hover:underline"
          >
            Sign Up
          </button>
        </p>
      </div>
    </div>
  );
};

export default LoginScreen;
