...
const NewYearChallengeScreen: React.FC<Props> = ({ onNavigate }) => {
  ...
  const { data: challenge } = useFirestoreDoc<{
    ...
    supportFundTitle?: string;
    supportFundDescription?: string;
  ...
  useEffect(() => {
    ...
    const getTimeLeft = () => {
      const startDate = typeof challenge.startAt?.toDate === 'function' ? challenge.startAt.toDate() : new Date(challenge.startAt);
    ...
    getTimeLeft();
    const timer = window.setInterval(getTimeLeft, 1000);
    return () => window.clearInterval(timer);
  }, [challenge?.startAt]);

  const handleShare = async () => {
    const shareData = {
      title: 'Tiizi Challenge',
      text: 'Join me in the Year of the Warrior challenge on Tiizi.',
    ...
    try {
      if (navigator.share) {
        ...
        addToast('Sharing is not supported on this device', 'error');
      }
    } catch {
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-background-light dark:bg-background-dark font-sans text-slate-900 dark:text-slate-100 antialiased pb-40">
      ...
      <header className="sticky top-0 z-50 flex items-center bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md px-5 pt-12 pb-4 justify-between border-b border-primary/10">
        ...
        <button onClick={handleShare} className="flex w-10 items-center justify-end text-primary">
      ...
      <main className="flex-1">
        {!challenge && (
          <div className="px-6 pt-10 text-center text-slate-400">Challenge details are not available yet.</div>
        )}
        {challenge && (
        <>
        {/* Hero Section */}
        <div className="relative px-6 pt-10 pb-12 overflow-hidden">
          ...
          <div className="relative z-10 text-center py-12 bg-gradient-to-b from-primary/[0.08] to-transparent rounded-[32px] border-2 border-primary/5 shadow-inner group">
            <div className="inline-flex items-center px-5 py-2 rounded-full bg-primary/10 text-primary text-[10px] font-display font-black uppercase tracking-[0.3em] mb-8 shadow-sm">
              ...
              {challenge.heroSubtitle || 'Live Launch'}
            </div>
            <h1 className="text-6xl font-display font-black text-slate-900 dark:text-white mb-4 leading-[0.8] tracking-tighter italic">
              {challenge.heroTitle || challenge.title}
            </h1>
            {challenge.tagline && (
              <p className="text-slate-500 dark:text-slate-400 text-sm font-bold uppercase tracking-widest mt-6 opacity-80">{challenge.tagline}</p>
            )}
          </div>
        </div>
            ...
            <div className="flex flex-col gap-10 relative z-10">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="bg-slate-50 dark:bg-zinc-800/50 p-6 rounded-[24px] flex flex-col items-center gap-1 shadow-sm border border-white/50">
                  <span className="text-primary font-display font-black text-4xl italic tracking-tighter">{challenge.daysTarget ?? '--'}</span>
                ...
                <div className="bg-slate-50 dark:bg-zinc-800/50 p-6 rounded-[24px] flex flex-col items-center gap-1 border-l-8 border-primary shadow-sm">
                  <span className="text-slate-900 dark:text-white font-display font-black text-3xl italic tracking-tighter leading-none">{challenge.repsTarget ?? '--'}</span>
        ...
        <div className="mb-12 px-6">
          <div className="flex items-center justify-between mb-6 px-1">
            ...
            {challenge.participantCount !== undefined && (
              <span className="text-primary text-[9px] font-black uppercase tracking-widest">{challenge.participantCount} Warriors Committed</span>
            )}
          </div>
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
            ...
            <div className="flex gap-3 items-center text-[10px] text-slate-400 font-black uppercase tracking-widest bg-white dark:bg-zinc-900 px-6 py-4 rounded-full shadow-sm border border-slate-50 dark:border-white/5 whitespace-nowrap">
              ...
              {challenge.participantNote || 'Participants are joining now'}
            </div>
          </div>
        </div>
          ...
          <div className="bg-primary/5 dark:bg-primary/10 rounded-[32px] p-10 border-2 border-primary/20 flex flex-col gap-6 relative overflow-hidden group">
            ...
            <div className="flex items-start justify-between relative z-10">
              <div className="flex-1 pr-6">
                <h5 className="text-slate-900 dark:text-white font-display font-black text-lg tracking-tight uppercase italic">{challenge.supportFundTitle || 'Support Fund'}</h5>
                {challenge.supportFundDescription && (
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-8 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-xl border-t border-slate-50 dark:border-white/5 z-50">
        <button 
          onClick={() => onNavigate((challengeId ? `${AppView.SETUP_CHALLENGE}?challengeId=${encodeURIComponent(challengeId)}` : AppView.SETUP_CHALLENGE) as AppView)}
